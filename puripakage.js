...
function userpack()
{
    ...
    console.log(data);
        ...
        if(item.email1==localemail)
        {
            ...
            let k=0;
            while(k<item.price.length){
                customer.push(item.customer[k]);
               
                k++;
            }
            customer.push(users.value);
           
            addpackage(item.id,arr,purchase,customer);
...
//add booked package
function addpackage(id,arr,purchase,customer){
    // alert('ok');
